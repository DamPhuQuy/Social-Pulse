...
import { getReports, updateReportStatus, type ReportResponse, type ReportStatus } from "@/services/social/reportService";
...
export default function AdminDashboard() {
  ...
  const loadStats = async () => {
    // Fetch stats by loading first page of each status
    const pendingRes = await getReports("PENDING", 0, 1);
    const resolvedRes = await getReports("RESOLVED", 0, 1);
    const rejectedRes = await getReports("REJECTED", 0, 1);
  ...
  const handleUpdateStatus = async (reportId: number, newStatus: "RESOLVED" | "REJECTED") => {
    const res = await updateReportStatus(reportId, newStatus);
    if (res.ok) {
      toast.success(newStatus === "RESOLVED" ? "Đã phê duyệt và xử lý báo cáo!" : "Đã bác bỏ báo cáo!");
      loadReports();
      loadStats();
    } else {
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-[260px_1fr] xl:grid-cols-[280px_1fr] gap-8 pt-24 px-6 lg:px-10">
        ...
        <div className="min-w-0 space-y-6 pb-10">
            ...
            <div className="flex items-center space-x-3 bg-white dark:bg-[#1e1e1e] p-6 rounded-2xl border border-slate-200/80 dark:border-[#2a2a2a] shadow-sm">
              ...
              <div>
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">
                  Kiểm duyệt, xem xét và xử lý các bài đăng hoặc tài khoản bị báo cáo vi phạm tiêu chuẩn cộng đồng.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-white dark:bg-[#1e1e1e] p-5 rounded-2xl border border-slate-200/80 dark:border-[#2a2a2a] shadow-sm flex items-center justify-between">
                <div>
                  <span className="text-slate-500 dark:text-slate-400 text-sm font-medium">Đã phê duyệt ẩn</span>
                        ...
                        <tr key={report.id} className="hover:bg-slate-50/50 dark:hover:bg-neutral-800/30 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${
                              report.targetType === "POST" ? "bg-purple-500/10 text-purple-500" :
                              report.targetType === "COMMENT" ? "bg-indigo-500/10 text-indigo-500" :
                          ...
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${
                              ...
                              {report.status === "PENDING" ? "Chờ xử lý" : report.status === "RESOLVED" ? "Đã ẩn" : "Bác bỏ"}
                            </span>
                          </td>
                            ...
                            {report.status === "PENDING" && (
                              <div className="inline-flex gap-2">
                                <button
                                  ...
                                  className="px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-semibold shadow-md shadow-green-600/10 transition"
                                >
                                  Ẩn mục
                                </button>
                                <button
                                  onClick={() => handleUpdateStatus(report.id, "REJECTED")}
                                  className="px-3 py-1.5 bg-slate-200 dark:bg-slate-800 hover:bg-slate-350 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-200 rounded-lg text-xs font-semibold transition"
                                >
                                  Bác bỏ
                                </button>
                              </div>
